...
from dqn_tools.memory import SimpleMemory, IOMemory
...
class BorsukDQNv0Template(ModelTemplate):
    ...
    def new_memory(self, working_path: str):
        return IOMemory(self.MEMORY_SIZE, self.MEMORY_PIECE, working_path, min_records=self.START_TRAINING_AT)
    ...
    def action(self, acting_model: keras.Model, models_memory, environment: cb.ChessBoard, epsilon):
        ...
        if np.random.uniform(0, 1) < epsilon:
            ...
            best_move, best_state, best_state_fen = self.choose_action(acting_model, moves, states, fens)
        # make move
        ...
        if training_batch is not None:
            ...
            for p, f in zip(prizes, fens):
                ...
                if not training_board.game_over():
                    ...
                    _, chosen_state, _ = self.choose_action(acting_model, next_moves, np.array(next_states), next_fens)
