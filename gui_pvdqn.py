...
class Window(QWidget):
            ...
            if event.buttons() == Qt.LeftButton:
                if self.svgX + self.margin < event.x() < self.svgX + self.board_size - self.margin and \
                    ...
                    if self.chosen_piece[0] is not None:
                        ...
                        legal_moves, _, _ = self.board.get_moves()
                        if move in legal_moves:
                            self.board.make_move(move, color=self.board.current_turn())
                            piece = None
                            coordinates = None
                            can_ai_move = self._can_next_player_move()
                            if can_ai_move:
                                ai_move, _ = self.ai_engine.choose_move(self.board, flip=True)
    ...
    def paintEvent(self, a0: QtGui.QPaintEvent):
        ...
        if self._is_game_over():
            if self.result == '1-0':
                king_attacked_by = self.board.get_king_attackers(False)
                self.text_label.setText("You have won!")
            elif self.result == '0-1':
                king_attacked_by = self.board.get_king_attackers(True)
